...
function createPlayer() {
    ...
    // Creazione della faccia come sprite separato ma legato a Mario
    this.face = this.add.sprite(player.x, player.y, 'faccia');
    this.face.setOrigin(0.5, 0.5);
    this.face.setScale(0.10); // Dimensione media
    this.face.depth = 6;
}
...
function updatePlayer(delta) {
    ...
    // Aggiorna posizione faccia - SEMPRE, anche quando playerBlocked è true
    if (this.face && player) {
        // Posizionamento dinamico in base allo stato di Mario
        let headX = player.x - 25; // Sposta parecchio a sinistra per centrare sulla testa
        let headY;
        let faceScale;
        ...
        if (playerState === 0) {
            // Mario piccolo
            headY = player.y - 50;
            faceScale = 0.10; // Scala normale
        } else if (playerState === 1) {
            // Mario grown
            headY = player.y - 70; // Più in alto perché è più grande
            faceScale = 0.12; // Leggermente più grande
        } else {
            // Mario fire
            headY = player.y - 70; // Più in alto perché è più grande
            faceScale = 0.12; // Leggermente più grande
        ...
        this.face.x = headX;
        this.face.y = headY;
        this.face.setScale(faceScale); // Aggiorna la scala in base allo stato
        
        // Gira la faccia con Mario
    ...
    // Tasti per testare i poteri di Mario (versione sicura)
    try {
        if (Phaser.Input.Keyboard.JustDown(controlKeys.GROW) && playerState < 1) {
            playerState = 1;
        }
        
        if (Phaser.Input.Keyboard.JustDown(controlKeys.FIRE_POWER) && playerState < 2) {
            playerState = 2;
        }
    } catch (e) {
        // Ignora errori per evitare blocchi
    }
